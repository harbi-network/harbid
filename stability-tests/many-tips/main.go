...
import (
	...
	"github.com/harbi-network/harbid/domain/consensus/utils/mining"
	...
	"github.com/harbi-network/harbid/stability-tests/common"
	"github.com/harbi-network/harbid/stability-tests/common/rpc"
...
func startNode() (teardown func(), err error) {
	...
	dataDir, err := common.TempDir("harbid-datadir")
	if err != nil {
		panic(errors.Wrapf(err, "Error in Tempdir"))
	}
	log.Infof("harbid datadir: %s", dataDir)

	harbidCmd, err := common.StartCmd("harbid",
		"harbid",
	...
	spawn("startNode-harbidCmd.Wait", func() {
		err := harbidCmd.Wait()
		if err != nil {
			if atomic.LoadUint64(&shutdown) == 0 {
				panics.Exit(log, fmt.Sprintf("harbidCmd closed unexpectedly: %s. See logs at: %s", err, dataDir))
			}
			if !strings.Contains(err.Error(), "signal: killed") {
				// TODO: Panic here and check why sometimes harbid closes ungracefully
				log.Errorf("harbidCmd closed with an error: %s. See logs at: %s", err, dataDir)
			}
		}
	...
	return func() {
		...
		killWithSigterm(harbidCmd, "harbidCmd")
...
func mineLoopUntilHavingOnlyOneTipInDAG(rpcClient *rpc.Client, miningAddress util.Address) error {
	...
	harbiMinerCmd, err := common.StartCmd("MINER",
		"harbiminer",
		common.NetworkCliArgumentFromNetParams(activeConfig().NetParams()),
		"-s", rpcAddress,
	...
	spawn("harbi-miner-Cmd.Wait", func() {
		err := harbiMinerCmd.Wait()
		if err != nil {
			if atomic.LoadUint64(&shutdown) == 0 {
	...
	atomic.StoreUint64(&shutdown, 1)
	killWithSigterm(harbiMinerCmd, "harbiMinerCmd")
	return nil
}
