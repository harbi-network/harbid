...
func startNodes() (teardown func(), err error) {
	...
	syncerDataDir, err := common.TempDir("harbid-datadir-syncer")
	if err != nil {
		panic(errors.Wrapf(err, "error in Tempdir"))
	}
	log.Infof("SYNCER datadir: %s", syncerDataDir)

	syncedDataDir, err := common.TempDir("harbid-datadir-synced")
	if err != nil {
		panic(errors.Wrapf(err, "error in Tempdir"))
	}
	log.Infof("SYNCED datadir: %s", syncedDataDir)

	syncerCmd, err := common.StartCmd("harbid-SYNCER",
		"harbid",
	...
	syncedCmd, err := common.StartCmd("harbid-SYNCED",
		"harbid",
		common.NetworkCliArgumentFromNetParams(activeConfig().NetParams()),
