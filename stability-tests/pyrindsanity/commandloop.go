...
func commandLoop(argsChan <-chan []string) ([]commandFailure, error) {
	...
	dataDirectoryPath, err := common.TempDir("harbidsanity-harbid-datadir")
	if err != nil {
	...
	for args := range argsChan {
		...
		cmd := exec.Command("harbid", args...)
		cmd.Stdout = common.NewLogWriter(log, logger.LevelTrace, "harbid-STDOUT")
		cmd.Stderr = common.NewLogWriter(log, logger.LevelWarn, "harbid-STDERR")
