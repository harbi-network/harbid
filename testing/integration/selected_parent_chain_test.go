...
func TestVirtualSelectedParentChain(t *testing.T) {
	// Setup a couple of harbid instances
	harbid1, harbid2, _, teardown := standardSetup(t)
	defer teardown()
	...
	err := harbid1.rpcClient.RegisterForVirtualSelectedParentChainChangedNotifications(true,
	...
	// In harbid2, mine a different chain of `blockAmountToMine` + 1
	// blocks over the genesis
	var chain2Tip *externalapi.DomainBlock
	for i := 0; i < blockAmountToMine+1; i++ {
		chain2Tip = mineNextBlock(t, harbid2)
	}

	// Connect the two harbids. This should trigger sync
	// between the two nodes
	connect(t, harbid1, harbid2)
	...
	// the first chain
	virtualSelectedParentChainFromChain1Tip, err := harbid1.rpcClient.GetVirtualSelectedParentChainFromBlock(
