...
func TestCreateDefaultConfigFile(t *testing.T) {
	...
	sampleConfigFile := filepath.Join(filepath.Dir(path), "sample-harbid.conf")

	// Setup a temporary directory
	tmpDir, err := ioutil.TempDir("", "harbid")
	if err != nil {
		t.Fatalf("Failed creating a temporary directory: %v", err)
	}
	testpath := filepath.Join(tmpDir, "test.conf")

	// copy config file to location of harbid binary
	...
	tmpConfigFile := filepath.Join(appPath, "sample-harbid.conf")
