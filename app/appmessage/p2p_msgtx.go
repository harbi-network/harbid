...
// Outpoint defines a harbi data type that is used to track previous
// transaction outputs.
...
// NewOutpoint returns a new harbi transaction outpoint point with the
...
// TxIn defines a harbi transaction input.
type TxIn struct {
...
// NewTxIn returns a new harbi transaction input with the provided
...
// TxOut defines a harbi transaction output.
type TxOut struct {
	Value        uint64
	ScriptPubKey *externalapi.ScriptPublicKey
}

// NewTxOut returns a new harbi transaction output with the provided
...
// MsgTx implements the Message interface and represents a harbi tx message.
