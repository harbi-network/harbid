...
import (
	...
	"github.com/harbi-network/harbid/infrastructure/os/execenv"
	"github.com/harbi-network/harbid/infrastructure/os/limits"
	"github.com/harbi-network/harbid/infrastructure/os/signal"
	"github.com/harbi-network/harbid/infrastructure/os/winservice"
...
var serviceDescription = &winservice.ServiceDescription{
	Name:        "harbidsvc",
	DisplayName: "harbid Service",
	Description: "Downloads and stays synchronized with the Harbi blockDAG and " +
		"provides DAG services to applications.",
}

type harbidApp struct {
	cfg *config.Config
}

// StartApp starts the harbid app, and blocks until it finishes running
func StartApp() error {
...
func (app *harbidApp) main(startedChan chan<- struct{}) error {
	...
	if err != nil {
		log.Errorf("Unable to start harbid: %+v", err)
		return err
	}

	defer func() {
		log.Infof("Gracefully shutting down harbid...")
		...
		log.Infof("harbid shutdown complete")
	}()
