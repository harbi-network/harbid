...
func send(conf *sendConfig) error {
	...
	if !conf.IsSendAll {
		sendAmountLeor = uint64(conf.SendAmount * constants.LeorPerHarb)
	}
	...
	for i, unsignedTransaction := range createUnsignedTransactionsResponse.UnsignedTransactions {
		signedTransaction, err := libharbiwallet.Sign(conf.NetParams(), mnemonics, unsignedTransaction, keysFile.ECDSA)
		if err != nil {
			return err
		}
