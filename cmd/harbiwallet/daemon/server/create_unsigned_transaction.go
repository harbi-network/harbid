...
//my-add		return nil, errors.Errorf("wallet daemon is not synced yet, %s", s.formatSyncStateReport())
	...
	unsignedTransaction, err := libharbiwallet.CreateUnsignedTransaction(s.keysFile.ExtendedPublicKeys,
...
func (s *server) selectUTXOs(spendAmount uint64, isSendAll bool, feePerInput uint64, fromAddresses []*walletAddress) (
	selectedUTXOs []*libharbiwallet.UTXO, totalReceived uint64, changeLeor uint64, err error) {
	...
	if dagInfo.NetworkName == "harbi-testnet" {
	...
	if totalValue < totalSpend {
		return nil, 0, 0, errors.Errorf("Insufficient funds for send: %f required, while only %f available",
			float64(totalSpend)/constants.LeorPerHarb, float64(totalValue)/constants.LeorPerHarb)
	}
