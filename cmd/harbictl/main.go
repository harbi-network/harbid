...
import (
	...
	"github.com/harbi-network/harbid/infrastructure/network/rpcclient/grpcclient"
)
...
func main() {
	...
	if !cfg.AllowConnectionToDifferentVersions {
		harbidMessage, err := client.Post(&protowire.HarbidMessage{Payload: &protowire.HarbidMessage_GetInfoRequest{GetInfoRequest: &protowire.GetInfoRequestMessage{}}})
		if err != nil {
			printErrorAndExit(fmt.Sprintf("Cannot post GetInfo message: %s", err))
		}

		localVersion := version.Version()
		remoteVersion := harbidMessage.GetGetInfoResponse().ServerVersion
...
func prettifyResponse(response string) string {
	harbidMessage := &protowire.HarbidMessage{}
	err := protojson.Unmarshal([]byte(response), harbidMessage)
	if err != nil {
	...
	return marshalOptions.Format(harbidMessage)
}
