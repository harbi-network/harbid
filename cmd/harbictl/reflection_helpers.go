...
// generateharbidMessage generates a wrapped harbidMessage with the given `commandValue`
func generateharbidMessage(commandValue reflect.Value, commandDesc *commandDescription) (*protowire.HarbidMessage, error) {
	commandWrapper := reflect.New(commandDesc.typeof)
	unwrapCommandValue(commandWrapper).Set(commandValue)

	harbidMessage := reflect.New(reflect.TypeOf(protowire.HarbidMessage{}))
	harbidMessage.Elem().FieldByName("Payload").Set(commandWrapper)
	return harbidMessage.Interface().(*protowire.HarbidMessage), nil
}
